...
def home(request):
    post=Article.objects.filter(is_active=True,status='published')[:8]
...
def post_list(request):
    ...
    post=Article.objects.filter(status='published',is_active=True)
    category_slug = request.GET.get('category')
    categories=Category.objects.filter(is_active=True)
    if category_slug:
        post = post.filter(category__slug=category_slug)
    if search:
        post=post.objects.filter(Q(name__icontains=search)| Q(description__icontains=search))
    if sort=='recent':
